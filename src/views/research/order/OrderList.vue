<template>
  <div>
    ...
    <el-table v-loading="dataLoading" :data="dataList" border fit highlight-current-row>
      <el-table-column prop="taskCode" label="任务编号" width="145"/>
      ...
      <el-table-column label="操作" align="center" width="160">
        <template slot-scope="{row}">
          <el-button type="text" size="mini">
            <router-link :to="'/research/editTask/'+row.taskCode">查看研判令</router-link>
          </el-button>
          <el-button :loading="row.loading" type="text" size="mini" @click="handleApplyClick(row)">申请加入</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
import {getCapableTask, getResearchObject, getTaskLevel, getTaskStatus} from '@/api/wisResTask'
import {submitApply} from '@/api/wisResApply'
...
export default {
  ...
  methods: {
    ...
    fetchCapableTask() {
      ...
      getCapableTask(this.query).then(resp => {
